...
@section('content')
    ...
    <h1 class="vacancies-heading section-top"
        style="background: url('{{ asset(setting('vacancy_image')) }}');  background-position: center center;
    background-repeat: no-repeat;
    background-size: cover;">
        @lang('trans.VACANCIES')</h1>
    <section class="contact-us--section section-top container">
        <h2>@lang('trans.VACANCIES')</h2>
        <p>
            @lang("trans.In OR you can find an exciting opportunity to explore job openings. It's a great way to discover new positions that might be available within our company. Keep an eye on this section for any updates and potential job opportunities that align with your interests and skills.")
        </p>
        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                @if ($vacancies->count() > 0)
                    <form method="POST" action="{{ route('client.career.post') }}" enctype="multipart/form-data"
                        class="contact-us-form">
                        @csrf
                        <select name="vacancy_id" class="form-select shipping-options">
                        ...
                        @error('email')
                            <p class="alert alert-danger" style='color:red;'>{{ $message }}</p>
                        @enderror
                        <textarea style="font-size: 2.3rem;" style="height: 500px" class="form-select " name="address"
                            value="{{ old('address') }}" placeholder="@lang('trans.address')"></textarea>
                        @error('address')
                            <p class="alert alert-danger" style='color:red;'>{{ $message }}</p>
                        @enderror
                        {{-- <select class="form-select shipping-options country">
                            ...
                            <select class="form-select shipping-options city">
                                <option>City</option>
                                <option value="1">1</option>
                                <option value="2">2</option>
                                <option value="3">3</option>
                        ...
                        <div class="w-100 d-flex flex-column w-100">
                            <input type="file" name="file" accept=".pdf" id="uploadBtn" />

                            <label for="uploadBtn" class="text-center upload-cv-btn w-auto">@lang('trans.UPLOAD CV')</label>
                            @error('file')
                                <p class="alert alert-danger" style='color:red;'>{{ $message }}</p>
                            @enderror
                            <button type="submit" class="btn w-auto">@lang('trans.APPLY')</button>
                        </div>

                    </form>
                @else
                   <p class="my-5">@lang('trans.No Vacancies Now')</p> 
                @endif
            </div>
