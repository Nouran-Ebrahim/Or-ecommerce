                    ...
                    <div class="panel">
                        <div class="panel-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped text-center" id="ConsutaBPM">
                                    ...
                                    <tbody>
                                        @if (count($orders) > 0)
                                            @foreach ($orders as $key => $order)
                                                <tr class="gradeX {{ $order['id'] }}">
                                                    ...
                                                    @if ($order->coupon > 0)
                                                        <td>{{ $order->sub_total_after_coupon }}
                                                            {{ Country()->currancy_code }}</td>
                                                    @else
                                                        <td>{{ $order->sub_total }} {{ Country()->currancy_code }}</td>
                                                    @endif
                                                    @php($order->coupon > 0?$sub_total += $order['sub_total_after_coupon']:$sub_total += $order['sub_total'])
                                        ...
                                        @else
                                            <tr>
                                                <td colspan="6" style="text-align: center!important;">@lang('trans.noElements')
                                                </td>
                                            </tr>
                                        @endif
                                    </tbody>
                            ...
                            <div class="row">
                                @if (count($orders) > 0)
                                    <div class="col-md-3 form-group" style="padding-top: 26px">
                                        <form class="row" action="{{ route('admin.exportData', ['data' => $orders]) }}"
                                            method="GET" style="display:contents">
                                            @csrf
