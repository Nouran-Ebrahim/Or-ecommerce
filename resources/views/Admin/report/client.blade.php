                            ...
                            <form class="row">
                                <div class="col-md-3 form-group">
                                    ...
                                    <select id="client_id" name="client_id" class="form-control">
                                        <option value="">---</option>
                                        @foreach($clients->sortBy('first_name') as $client2)
                                            <option {{ $client2['id'] == request('client_id') ? 'selected' : '' }} value="{{ $client2['id'] }}">{{ $client2['first_name'].' '.$client2['last_name'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    @if($client && count($client->orders) > 0)
                                        @foreach($client->orders as $key => $order)
                                            <tr class="gradeX {{ $order['id'] }}">
                                                ...
                                                <td style="text-align:center;">{{ $order['net_total'] }} {{  Country()->currancy_code }}</td>
                                                <td style="text-align:center;">{{ $order->PaymentMethod['title_' . app()->getLocale()] }}</td>
