...
<form method="POST" action="{{ route('admin.banner.update',['banner' => $Image ]) }}" enctype="multipart/form-data" data-parsley-validate novalidate>
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            <select class="form-control" required id="section" name="postion">
                <option {{ $Image['postion'] == "1" ? 'selected' : '' }} value="sec1">@lang('trans.section1')</option>
                <option {{ $Image['postion'] == "2" ? 'selected' : '' }} value="sec2">@lang('trans.section2')</option>
            </select>
        </div>
        <div class="form-group col-md-6">
