...
<form method="POST" action="{{ route('admin.branches.update',$Branch) }}" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            <input type="text" name="whatsapp" id="whatsapp" class="form-control"  value="{{ $Branch['whatsapp'] }}">
        </div>
        <div class="form-group col-md-6">
        ...
        <div class="form-group col-md-6">
            ...
            <select class="form-control selectpicker border"  data-live-search="true"  id="region_id" name="region_id">
                ...
                @foreach ($regions as $region)
                    <option {{ $region->id == $Branch->region_id ? 'selected' : '' }} value="{{ $region->id }}">{{ $region->title() }}</option>
                @endforeach
            </select>
        </div>
        {{-- <div class="form-group col-md-6">
            <label for="categories">@lang('trans.categories')</label>
            <select class="form-control selectpicker border" id="categories" required multiple name="categories[]">
                @foreach ($categories as $category)
                <option value="{{ $category->id }}" @selected(in_array($category->id, $Branch->categories->pluck('id')->toarray()))>{{ $category->title() }}</option>
                @endforeach
            </select>
        </div> --}}
        {{--
        <div class="form-group col-md-6">
            ...
            <select class="form-control selectpicker border" id="products" required multiple name="products[]">
                ...
                <option value="{{ $product->id }}"  @selected(in_array($product->id, $Branch->products->pluck('id')->toarray()))>{{ $product->title() }}</option>
                @endforeach
            </select>
        </div>
        --}}
        ...
        <div class="openclose text-center col-12 mx-auto">
            ...
            @foreach ($Branch->Days->sortBy('id') as $Day)
                <div class="row">
                    <div class="col-md-4 col-sm-12">
                        <label>@lang('trans.day')</label>
                        <input value="{{ $Day->id }}" readonly class="form-control" name="days[]" type="hidden" >
                        <input value="{{ $Day->title() }}" readonly class="form-control" type="text" >
                    </div>
                    <div class="col-md-3 col-sm-12">
                        <label>@lang('trans.open')</label>
                        <input value="{{ $Day->pivot->open }}" class="form-control" name="open[]" type="text" required="">
                    </div>
                    <div class="col-md-3 col-sm-12">
                        <label>@lang('trans.close')</label>
                        <input value="{{ $Day->pivot->close }}" class="form-control" name="close[]" type="text" required="">
                    </div>
